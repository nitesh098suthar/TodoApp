...
export const forgetController = async (req, res) => {
    ...
    await UserModel.findByIdAndUpdate(userAvailable._id, { resetToken: token, resetTokenExpire: new Date(Date.now() + 5 * 60 * 1000) })
...
export const resetController = async (req, res) => {
    ...
    const user = await UserModel.findOne({ resetToken: token, resetTokenExpire: { $gt: Date.now() } }).select("+password")
    if (!user) {
        return res.status(404).json({
            success: false,
            message: "not an user"
